...
本项目是由用户深度定制的 **发烧级桌面分体式纯音数播解码前级系统**。全系统采用 **树莓派 5 代 (1GB)** 作为数字交互中枢，基于 **Modern C++20** 原生自研纯音播放器核心，开机 3.8 秒直入 **60fps 经典麦景图（McIntosh）湖蓝色动圈双表头** 交互界面，硬件级 Bit-Perfect 零抖动直出至 **AK4191EQ + AK4499EX 旗舰全平衡解码前级**，驱动 **山水 AU-α707KX 功放** 与 **惠威 M3 旗舰书架箱**。
...
>    * **Box 2: 纯模拟解码前级（后级端）**：AK4191EQ + AK4499EX 旗舰软控全平衡板 + 英国箭猪 50VA 环牛 (双15V 模拟) + 庆达全铜 R-20 变压器 (双9V 数字时钟) + 万可免焊端子。**采用独立全铝纤薄金属机箱**（全金属屏蔽 220V 市电与电磁辐射，声底黑如深空）。
...
> 4. **I2S 异步从机直出**：树莓派仅出 3 线制 I2S（BCLK/LRCLK/DOUT）+ 1 根参考地 GND，严禁折腾 MCLK 飞线（AK4499EX 成品板载双飞秒晶振自同步，彻底隔绝树莓派时钟抖动）。
...
flowchart LR
    ...
    subgraph DACBox ["Box 2: 纯模拟解码前级 (全铝金属屏蔽机箱 · 纯线性双变压器)"]
        DAC["AK4191EQ + AK4499EX 软控全平衡板<br>(双飞秒晶振 / 数模彻底分离 / Velvet Sound)"]
...
```
 ...
 ★ 7按键公共地  Pin 9 (GND)    ───┤ Pin 9  Pin 10│
   DOWN 向下键  Pin 11 (GPIO 17) ──┤ Pin 11  Pin 12│─────── ➔ I2S BCLK (位时钟) ➔ 接 AK4499EX
  ...
  RIGHT 向右键  Pin 15 (GPIO 22) ──┤ Pin 15  Pin 16│──┐
     ...
     OK 确认键  Pin 29 (GPIO 5) ───┤ Pin 29  Pin 30│
                                   │ Pin 31  Pin 32│
                                   │ Pin 33  Pin 34│
     AK4499EX  I2S LRCLK (帧同步) ├── Pin 35  Pin 36│
                                   │ Pin 37  Pin 38│
     AK4499EX  数字音频参考地 GND ├── Pin 39  Pin 40│─────── ➔ I2S DOUT (数据) ➔ 接 AK4499EX
                                   └──────────────┘
...
2. **第二阶段（音质核心）：¥1,153.80**
   * AK4191EQ + AK4499EX 旗舰软控全平衡成品板（数模彻底分离，AKM Velvet Sound VERITA 旗舰架构，出厂配齐芯片、双时钟、运放与OLED屏）
